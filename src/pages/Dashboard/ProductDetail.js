...
const ProductDetail = ({ product }) => {
    return (
        <div className='flex flex-col 2xl:flex-row'>
            <div className='border border-slate-200 rounded w-56 2xl:w-96'>
                <img className='w-full h-full' src={product.productImage} alt="" />
            </div>
            <div className='2xl:px-6'>
                <h1 className='text-2xl font-semibold text-left'>{product.productName}</h1>
                <Rating
                    className='text-xs text-amber-400 2xl:my-2'
                    initialRating={product.review}
                    emptySymbol={<StarOutlined />}
                    fullSymbol={<StarOutlined />}
                    readonly>
                </Rating>
                <p className='flex text-xs'><span className='mr-2'>AVAILABILITY :</span>
                ...
                <p className='flex text-xs'><span className='mr-2'>ID :</span> <span>{product._id}</span></p>
